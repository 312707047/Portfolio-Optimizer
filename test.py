...
class Agent(object):
    def __init__(self, **kwargs):
        ...
        self.critic = Critic(s_dim+a_dim, 512, a_dim).to(self.device)
        self.critic_target = Critic(s_dim+a_dim, 512, a_dim).to(self.device)
    ...
    def act(self, s0):
        s0 = torch.tensor(s0, dtype=torch.float, device=self.device).unsqueeze(0)
